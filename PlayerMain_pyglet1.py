...
class PlayerMain(pyglet.window.Window):
    def __init__(self):
        super(PlayerMain, self).__init__(internalResolutionX, internalResolutionY)
...
#        createCvWindow(fullscreenMode, self._internalResolutionX, self._internalResolutionY, positionX, positionY)
        ...
        emptyImage = getEmptyImage(self._internalResolutionX, self._internalResolutionY)
        self._pygletImage = pyglet.image.ImageData(self._internalResolutionX, self._internalResolutionY, "BGR", emptyImage.tostring())
        self._videoSprite = pyglet.sprite.Sprite(self._pygletImage)
        pyglet.clock.schedule_interval(self._updateFrame, 1.0/60)
...
#        cv.CvtColor(mixedImage, mixedImage, cv.CV_BGR2RGB)
        ...
        self._pygletImage.set_data("BGR", self._internalResolutionX*3, rawData)
#        self._pygletImage = pyglet.image.ImageData(self._internalResolutionX, self._internalResolutionY, "BGR", rawData)
...
#                self._log.info("Too slow main schedule " + str(dt))
        print "d",
        self.clear()
        self._videoSprite.image = self._pygletImage
        self._videoSprite.draw()
